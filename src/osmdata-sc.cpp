...
//' @noRd
// [[Rcpp::export]]
Rcpp::List rcpp_osmdata_sc_infile (const std::string& file_path)
{
    ...
    if (len < 0)
        Rcpp::stop ("Failed to determine size of file: " + file_path);
    file.seekg (0, std::ios::beg);

    // Single allocation + single read: avoids the extra full-size copy made by
    // a stringstream buffer, which matters for multi-GB extracts.
    std::string st;
    st.resize (static_cast <size_t> (len));
    if (len > 0)
        file.read (&st [0], len);
    if (file.bad ())
        Rcpp::stop ("Failed to read file: " + file_path);
    file.close ();

    return rcpp_osmdata_sc (st);
}
